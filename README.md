# NPCAgent - Project Overview - [Demo](https://psychosmiley.github.io/NPCAgent/)

A deliberately **minimal LLM simulation proving NPC emergence by unseeded** agents. Three agents that move through a Park / Home / Cafe sandbox, run a small circular economy, and remember through one lossy diary. Function-based, no classes, a single Python file that runs on many LLMs (even old ones like llama2) with no GPT-4, no vector embeddings, no huge context window. Inspired by `joonspk-research/generative_agents` (`Generative Agents: Interactive Simulacra of Human Behavior` 1y-6ppl) and `a16z-infra/ai-town`, but dev solo and focused on simplicity and incremental features: persistent, believable agents that **share information and interact with their environment.**
...
<details>
<summary><b>The Philosophy</b></summary>

I started this in 2023 with one idea, [inspired by Stanford's generative agents](https://github.com/joonspk-research/generative_agents): an NPC could run end-to-end on a local model, *no GPT-4 or external call*, and not just for chat. No stat sheet propping it up, no giant seeded-prompt framework. I wanted them to feel alive as a self being, part of a living world, and still be worth talking to.
...
- Emergence lives in TRANSITIONS - drama is balance lost and re-earned, never balance held: a solved world offers only ritual-loop or extinction, and a smarter model just reaches one faster. Births/deaths keep re-posing the solved world; the god of balance is a moving target
...
- the `50 msg × 100 tok = 5000 tok` window fits everything from 8K-context locals to 32k+ flagships, over any OpenAI-compatible API
...
- notes surface in the morning reminder at the end of history where attention is strongest, injected once at wake, then age out of history naturally (avoids redundancy + overflow)
...
[Answer/ChatbotAI] role: "Bob": `{"response": "Alice, have you spoken to anyone else in this AI town? I would love to meet them!...`
```
